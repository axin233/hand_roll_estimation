...
class ResCNNEncoder(nn.Module):
    def __init__(self, weight_dir, training=True):
        ...
        # Load the weights when training the network
        if training:
        ...
        # Freeze the network during training
        if training:
...
class DecoderRNN(nn.Module):
    def __init__(self, CNN_embed_dim=300, h_RNN_layers=3, h_RNN=256, h_FC_dim=128, drop_p=0.3, output_dim=50):
        ...
        self.output_layer = nn.Linear(self.h_FC_dim, self.output_dim)#test
    ...
    def forward(self, x_RNN):
        ...
        # FC layers
        x = self.fc1(RNN_out)   #RNN_out.shape=(batch_size,timestep,h_RNN)
        x = F.relu(x)
