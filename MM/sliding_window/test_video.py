...
# -*- coding: utf-8 -*-
"""
Created on Fri Nov 17 11:54:38 2023
    ...
    with torch.no_grad():
        ...
        with tqdm(data_loader_test, unit='batch') as tepoch:
            ...
            for sample_batch in tepoch:
                ...
                init_labels = sample_batch[1].to(device) #init_labels.shape=(B,T,2)
                current_frame_num=sample_batch[-1]#len(current_frame_num)=1
                # Cast the data type
                labels = init_labels.type(torch.float32)
                
                # Inference
                cnn_output=cnn_encoder(inputs, cnn_output, bool_first_batch) # Output size: (B, T, 64)
                rnn_output = rnn_decoder(cnn_output) # rnn_output.shape=(B,T,2)
                
                # Reset parameters
                bool_first_batch=False
                ...
                csv_output.write('{},{},{},{},{},{},{}\n'.format(current_frame_num[0], np_labels_deg[0], \
                    np_outputs_deg[0], np_labels[0,-1,0], np_labels[0,-1,1], np_outputs[0,-1,0], np_outputs[0,-1,1])) 
        ...
        #print('Test Loss (degree): {:.4f}'.format(avg_loss_deg))
...
if __name__ == '__main__':
    ...
    work_dir='/home/jianxig/CRNN/v2_1/l1/many-to-many/proposal/'
    ...
    cnn_encoder = network_many_to_many.ResCNNEncoder('', training=False)
    ...
    for data_collection_location in list_data_collection_location:
        ...
        for i, file_name in enumerate(list_subject_id):
            ...
            if int(split_min_HSV[0])==0 and int(split_min_HSV[1])==0 and int(split_min_HSV[2])==0:
                ...
                # 
                test_dataset = DIY_data_loader_many_to_many.SequenceDataset(sin_test, cos_test, area_num_test, \
