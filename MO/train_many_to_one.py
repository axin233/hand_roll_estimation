...
def train_model(device, data_loader, model, loss_function, optimizer, dataset_sizes_dict):
    ...
    with tqdm(data_loader, unit='batch') as tepoch:
        ...
        for batch_idx, sample_batch in enumerate(tepoch):
            ...
            y = sample_batch[1]#y.size: [B,2]
            X, y = X.to(device), y.to(device)
            optimizer.zero_grad()
            cnn_output = cnn_encoder(X)#cnn_output.size: [B,T,64]
            rnn_output = rnn_decoder(cnn_output)#rnn_output.size: [B,2]
            ...
            np_labels=y.cpu().detach().numpy()#np_labels.shape=(32, 2)
            np_labels_rad=np.arctan2(np_labels[:,0], np_labels[:,1])#np_labels_rad.shape=(32,)
            np_labels_deg=np_labels_rad*180/np.pi
            np_outputs=rnn_output.cpu().detach().numpy()#np_outputs.shape=(32,2)
            np_outputs_rad=np.arctan2(np_outputs[:,0], np_outputs[:,1])#np_outputs_rad.shape=(32,)
            np_outputs_deg=np_outputs_rad*180/np.pi
            np_offset_deg=batch_offset(np_outputs_deg, np_labels_deg)#np_offset_deg.shape=(32,)
            # MAE
    ...
    with torch.no_grad():
        ...
        with tqdm(data_loader, unit='batch') as tepoch:
            for batch_idx, sample_batch in enumerate(tepoch):
                ...
                y = sample_batch[1] # y.shape: (B, 2)
                X, y = X.to(device), y.to(device)
                cnn_output=cnn_encoder(X) # Output size: (B, T, 64)
                rnn_output = rnn_decoder(cnn_output) # Output size: (B, 2)
                
                # Loss calculation
                loss = loss_function(rnn_output, y.float())
                total_loss += (loss.item())*y.size(0)
                # Calculate the loss in degrees
                np_labels=y.cpu().detach().numpy()#np_labels.shape=(B,2)
                np_labels_rad=np.arctan2(np_labels[:,0], np_labels[:,1])#np_labels_rad.shape=(B,)
                np_labels_deg=np_labels_rad*180/np.pi
                np_outputs=rnn_output.cpu().detach().numpy()#np_outputs.shape=(B,2)
...
if __name__=='__main__':
    ...
    input_img_dir='/project/regroff/jianxig/img_dataset/'
    input_dir= '/project/regroff/jianxig/dataset_v11/CRNN/'
    output_dir='/home/jianxig/CRNN/v2_1/l1/many-to-one/proposal/'
    ...
    train_dataset = DIY_data_loader_many_to_one.SequenceDataset(sin_train, cos_train, area_num_train, \
    ...
    validation_dataset = DIY_data_loader_many_to_one.SequenceDataset(sin_validation, cos_validation, area_num_validation, \
    ...
    cnn_encoder = network_many_to_one.ResCNNEncoder(CNN_pretrained_weight_dir, training=True).to(device)
    ...
    for idx_epoch in range(EPOCH):
        ...
        if validation_loss < best_loss:
            ...
            #best_optimizer_dir=output_dir+'best_optimizer(many_to_one)(epoch {}).pt'.format(idx_epoch+1)
