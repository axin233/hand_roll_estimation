...
class SequenceDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if index<self.dataset_size and self.area_num[index]!=self.area_num[index+self.time_step-1]:
            ...
            network_target_sin=self.np_sin[index+self.time_step-1]
            network_target_cos=self.np_cos[index+self.time_step-1]
            network_target=np.asarray([network_target_sin, network_target_cos]).astype('float')#Target
